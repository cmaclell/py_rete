...
class HtnMethod:
    def __init__(self, name, task, args, conditions, subtasks=[]):
        self.name = name
        self.task = task
        self.args = args
...
class HtnAction:
    def __init__(self, name, task, args, conditions, effects):
        self.name = name
        self.task = task
        self.args = args
        self.conditions = conditions
...
class ReteHTN(ReteNetwork):
    pass

class HtnAction(HtnMethod):

    def __init__(self, name: str, task: str, args: List[V],
            conditions: Optional[Union[ConditionalElement, ConditionalList]] = None,
            effects):
        self.name = name
        self.task = task
    ...
    def fire(self, token: Token):
        for f in self.effects:
            if isinstance(f, NOT):
                for fact_id in self._rete_net.facts:
                    other = self._rete_net.facts[fact_id]
                    new_fact = Fact(**{attr: token.binding[f[attr]] if f[attr]
                        in token.binding else f[attr] for attr in f[0]})

                    keys = set()
                    keys.update(new_fact)
                    keys.update(other)
                    ...
                    if matches:
                        self._rete_net.remove_fact(other)
            else
                self._rete_net.add_fact(Fact(**{attr: token.binding[f[attr]] if
                    f[attr] in token.binding else f[attr] for attr in f}))

        return True

        # result = [Fact(**{attr: token.binding[f[attr]] if f[attr] in
        #     token.binding else f[attr]for attr in f}) for f in
        #     self.effects]
        # return result


class HtnMethod(Production):
    """
    ...
    def fire(self, token: Token):
        subtasks = [(task[0], {e: token.binding[e] for e in task[1:] if e in
            token.binding}) for task in subtasks]

        while len(subtasks) > 0:
            task, bindings = subtasks.pop(0)

            success = False
            for method in self._rete_net.tasks[task]:
                if success:
                    break

                for pnode in method.p_nodes:
                    if success:
                        break
    ...
    def __call__(self, *args, **kwargs):
        raise NotImplemented("Can't wrap functions with HTN Methods")

    def __repr__(self) -> str:
        return "{}({}): IF {} THEN {}".format(self.task, ", ".join(self.args),
                                            self.conditions.__repr__(),
                                            self.subtasks)

    def __eq__(self, other: object) -> bool:
        return (isinstance(other, HtnMethod) and self.id == other.id)
...
class ReteNetwork:
    ...
    def add_production(self, prod: Production) -> None:
        ...
        if isinstance(prod, (HtnMethod, HtnAction)):
            self.tasks[prod.task].append(prod)
    ...
    def remove_production(self, prod: Production) -> None:
        ...
        if isinstance(prod, (HtnMethod, HtnAction)):
            self.tasks[prod.task].remove(prod)
